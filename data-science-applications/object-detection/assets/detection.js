IMAGE_INTERVAL_MS = 42;

const drawObjects = (video,canvas, detections) => {
    ...
    ctx.beginPath();
    // we first clean everything to remove the rectangles from the previous detection
    ctx.clearRect(0, 0, ctx.width, ctx.height);
    for (const object of detections.objects) {
...
const startObjectDetection = (video, canvas, cameraId) => {
...
window.addEventListener("DOMContentLoaded", (event) => {
    ...
    document.getElementById("form-connect").addEventListener("submit", (event) => {
        ...
        const cameraId = cameraSelect.selectedOptions[0].value;
        socket = startObjectDetection(video, canvas, cameraId);
    });
});